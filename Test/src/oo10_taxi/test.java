...
public class test {
	public static void main(String[] args){
		...
		int i = 0;
		for(;i < 70;i++){
			taxi[i] = new Taxi(i,rd.nextInt(6400),RQ,map.getMap(),map.getWay(),buf,map,map.getLight_NS(),map.getLight_EW());
			taxi[i].start();
		}
		Buffer Nbuf = new Buffer(map.getXroadC(),map.getYroadC());
		Nbuf.start();
		for(;i < 100;i++){
			taxi[i] = new VipTaxi(i,rd.nextInt(6400),RQ,map.getMapC(),map.getWay(),Nbuf,map,map.getLight_NS(),map.getLight_EW());
			taxi[i].start();
		}
